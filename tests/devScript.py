...
class ParseTree():
    ...
    def grow(self, chosen_prod):
        tokens = re.findall(
            r"np\.tanh\(<e>\)|np\.sin\(<e>\)|<\w+>|x\[\d+\]|\.|{}".format('|'.join(self.terminals)), chosen_prod)
...
parse_tree = ParseTree(node_meta=reg_meta, grammar=Reg_BNF_GRAMMAR)
parse_tree.grow(reg_morph_steps[0][0])
for step, idx in reg_morph_steps[1:]:
    parse_tree.set_expansion_node(idx)
    parse_tree.grow(step)
    parse_tree.display()
...
# eval_tree = EvalSymRegTree('./simpleReg.json', Reg_BNF_GRAMMAR)
eval_tree = EvalSymRegTree('./simpleReg.json', Reg_BNF_GRAMMAR)
eval_tree.evaluate(parse_tree.tree)
